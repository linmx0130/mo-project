...
- `crates/mo_core/` — shared types, JSONL journal, SQLite DB (WAL)
- `crates/mo_gateway/` — axum HTTP service (port 3000), SSE, process mgmt
...
npm run lint
npm run build
npm run dev                   # dev server, /api proxied to :3000
```
...
- Worker env (model URL/name/token) is inherited from the gateway process:
  set `MO_MODEL_BASE_URL`/`MO_MODEL_NAME` on the gateway before `cargo run`.
